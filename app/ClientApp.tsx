...
export default function MeditationApp() {
  ...
  // Reset total seconds when minutes changes (only when idle)
  useEffect(() => {
  ...
  const chime = () => {
    ...
    try {
      ...
      o.type = "sine";
      o.frequency.value =
        phase === "inhale" ? 523.25 : phase === "hold" ? 659.25 : 392; // C5/E5/G4
      g.gain.value = 0.001;
  ...
  const start = () => {
    ...
    // phase countdown + cycling
    phaseTickRef.current = setInterval(() => {
      setPhaseSecondsLeft((p) => {
        if (p > 1) return p - 1;

        // switch phase when the last second of current phase is consumed
        setPhase((prev) => {
        ...
        // the callback must return a number
        return 0;
      });
    }, 1000);
  };
  ...
  const stop = (completed = false) => {
    ...
    // Save a session if we actually ran anything
    if (completed || secondsLeft < minutes * 60) {
      const duration = minutes * 60 - secondsLeft;
      setHistory((h) => [
        ...h,
        {
  ...
  // Auto-finish when total time hits 0
  useEffect(() => {
    if (isRunning && secondsLeft <= 0) stop(true);
  ...
  // Streak (days in a row incl. today)
  ...
  const weekTotalMin = useMemo(() => {
    ...
    return Math.round(
      history
        .filter((h) => new Date(h.startedAt) >= start)
      ...
      <main className="mx-auto max-w-5xl px-4 py-8">
        <header className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <h1 className="text-3xl sm:text-4xl font-semibold">
            CalmTrail • Guided Breathing
          </h1>
          <div className="flex flex-wrap gap-3 text-sm">
            <span className="rounded-xl bg-white/10 px-3 py-1">
              Streak: <b>{streak}</b> day{streak === 1 ? "" : "s"}
            </span>
            <span className="rounded-xl bg-white/10 px-3 py-1">
              This week: <b>{weekTotalMin}</b> min
            </span>
        ...
        <section className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl bg-white/10 p-4 backdrop-blur">
              ...
              {[5, 10, 15, 20].map((m) => (
                <button
                  ...
                  className={`rounded-xl px-3 py-2 ${
                    minutes === m ? "bg-white text-black" : "bg-white/10"
                  }`}
                >
                  {m} min
                </button>
            ...
            <div className="mt-4 text-5xl tabular-nums text-center">
              {fmt(secondsLeft)}
            </div>
          ...
          <div className="rounded-2xl bg-white/10 p-4 backdrop-blur sm:col-span-2">
            ...
            <p className="opacity-80">
              Follow the prompt. The circle expands as you inhale and contracts as you
              exhale.
            </p>
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2">
              <div className="flex flex-col items-center justify-center">
                ...
                <BreathCircle
                  key={`${phase}-${isRunning}-${pattern.inhale}-${pattern.hold}-${pattern.exhale}`}
                  phase={phase}
                  running={isRunning}
                  pattern={pattern}
                />
              ...
              <div>
                  ...
                  {(["inhale", "hold", "exhale"] as const).map((k) => (
                    <div key={k} className="rounded-xl bg-white/10 p-2 text-center">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setPattern({ ...pattern, [k]: Number(e.target.value || 0) })
                        }
                ...
                <div className="mt-3 grid grid-cols-3 gap-2 text-sm">
                  <button
                    disabled={isRunning}
                    onClick={() => setPattern({ inhale: 4, hold: 4, exhale: 4 })}
                    className="rounded-xl bg白/10 px-3 py-2"
                  >
                    Box 4-4-4
                  </button>
                  <button
                    disabled={isRunning}
                    onClick={() => setPattern({ inhale: 4, hold: 7, exhale: 8 })}
        ...
        <section className="mt-8 rounded-2xl bg-white/10 p-4 backdrop-blur">
            ...
            <button
              onClick={() => {
                ...
                const rows = history.map(
                  (r) =>
                    `${r.startedAt},${r.durationSec},${(r.durationSec / 60).toFixed(1)},${
                      r.pattern
                    }`
                );
                ...
                a.href = url;
                a.download = "meditation-history.csv";
                a.click();
          ...
          {history.length === 0 ? (
            <p className="opacity-80">No sessions yet. Hit Start and breathe.</p>
          ) : (
        ...
        <footer className="mt-8 text-center text-xs opacity-70">
          Built with Next.js + Tailwind. Data stays in your browser. Add to Home Screen to
          use like an app.
        </footer>
      </main>
    </div>
...
function BreathCircle({
  ...
  return (
    <div className="mt-4 flex h-64 w-64 items-center justify-center">
      <div
        className="relative grid place-items-center rounded-full border border-white/30 bg-white/5 shadow-2xl"
        style={{ width: 240, height: 240 }}
      >
        <div
          className={`relative z-10 grid place-items-center rounded-full bg-white/20 backdrop-blur transition-all duration-1000 ${
